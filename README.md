...
| Checklist | ✅ Migrado — hoje, atrasadas (com justificativa), próximos 5 dias (com justificativa de antecipação), todas as periodicidades (incl. Mensal/Quinzenal, com dias da semana configuráveis em todas elas), foto obrigatória, tempo real, tela "Gerenciar Checklist" (CRUD + reordenar + vínculo com Mapa/POP/Ficha). O picker de vínculo consulta as tabelas reais (`mapas_fluxogramas`/`pops`/`fichas_tecnicas`/`fichas_producao`) — Mapas e POP's ainda ficam vazios (sem tela própria ainda), mas Fichas Técnicas/Produção já aparecem de verdade. "Esta tarefa envolve produção" está completo: registra o lote, dá entrada automática no Estoque, reverte tudo se a tarefa for desmarcada, e oferece enfileirar etiqueta se houver impressora ativa |
...
| Mapas e Fluxogramas, POP's, Freelancer | ⬜ Ainda no protótipo — schema e RLS já existem nas migrations, falta a tela React |
